...
class ReleaseProtocolTests(unittest.TestCase):
    ...
    def test_install_uses_verified_assets_and_command_from_uv_tool_bin(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(receipt.installer_version, release.INSTALLER_VERSION)
            self.assertEqual(release.read_receipt(release.receipt_path(Path(temporary) / "state")).installer_version, release.INSTALLER_VERSION)
