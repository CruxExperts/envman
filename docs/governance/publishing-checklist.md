...
Use this checklist for the Envman 0.1.4 release. Every box is intentionally unchecked: local preparation is evidence for the controller, while GitHub and publication steps require an explicit remote review.
...
- [ ] The changelog has a `0.1.4` section with release notes that describe only shipped behavior.

## Local source and documentation checks

- [ ] `VERSION`, the README version display, package metadata, and the intended `v0.1.4` tag agree.
...
- [ ] After copying the wheel, source archive, and `install.py` into `release/`, run `uv run --locked --no-sync python scripts/release_assets.py --output release --version 0.1.4` and inspect `release-manifest.json`, `runtime-constraints.txt`, and `SHA256SUMS.txt`.
...
- [ ] Install the wheel in an isolated temporary uv-tool environment whose bin directory is absent from `PATH`, confirm `envman --version` reports `0.1.4` through the executable resolved by `uv tool dir --bin`, and confirm the private receipt records the verified manifest, assets, `installer_version: 0.1.4`, and uv version.
...
- [ ] Review the extracted `0.1.4` changelog notes and expected asset set before tagging; the workflow publishes automatically and the resulting release is immutable.
- [ ] Confirm the release commit is reachable from `main`, then create the version-matching tag `v0.1.4` only after all local checks pass.
- [ ] Push `v0.1.4` and confirm the tag-triggered GitHub workflow verifies the tag/VERSION match and `origin/main` ancestry.
...
- [ ] Confirm the publish job creates a draft from the `0.1.4` changelog section, attaches every intended asset, records build-provenance attestations, and then publishes the release automatically.
